...
<mjml lang="nl">
  ...
  <mj-body>
    <mj-section>
      <mj-column>
        <mj-text font-size="22px">Planning {{groupNameUp}}, week {{week}}</mj-text>
        <mj-divider />
        {{#unless complete}}
          <mj-text mj-class="alert">Nog open taken!</mj-text>
        {{/unless}}
        <mj-text mj-class="msg">
          Voor wijzigen en/of aanvullen, ga naar
        ...
        {{#each days}}
          <mj-divider />
          {{#if this.complete}}
            <mj-text mj-class="date">{{this.dateStr}}</mj-text>
          {{else}}
            <mj-text mj-class="date-alert">{{this.dateStr}}</mj-text>
          {{/if}}
          {{#each this.tasks}}
            {{#if this.complete}}
              <mj-text mj-class="task">
            {{else}}
              <mj-text mj-class="task-alert">
            {{/if}}
            {{this.startStr}} - {{this.endStr}}
            {{#if this.comment}}
              [{{this.comment}}]
            {{/if}}:
            {{#each this.volunteers}}
        ...
        <mj-text mj-class="sub">
          Uitschrijven
          van deze overzichts-emails kan in je
          "email notificaties" instellingen van
          je account op
          <a href="https://ijsplanner.be">ijsplanner.be </a>
        </mj-text>
