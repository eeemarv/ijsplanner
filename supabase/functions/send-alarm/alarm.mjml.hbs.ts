...
<mjml lang="nl">
  <mj-head>
    <mj-title>Alarm {{ groupName }}</mj-title>
    <mj-attributes>
  ...
  <mj-body>
    <mj-section>
      <mj-column>
        <mj-text mj-class="alert">Alarm!</mj-text>
        <mj-text mj-class="msg"> Morgen, {{timeStart}} van {{ timeEnd }} tot {{ endStr }} is een {{groupName}}-taak {{#if comment}}[{{comment}}] {{/if}}nog niet of nog niet compleet ingevuld. </mj-text>
        <mj-text mj-class="msg">Kan jij helpen?</mj-text>

        <mj-divider />

        <mj-text mj-class="sub">
          Uitschrijven van deze alarm-emails kan in de "email notificaties" instellingen van
